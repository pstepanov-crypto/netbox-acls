...
class ACLStandardRuleFilterSet(NetBoxModelFilterSet):
    ...
    # Source - ТОЛЬКО текстовые поля, НЕТ ForeignKey к Device
    source_prefix = django_filters.CharFilter(
...
class ACLExtendedRuleFilterSet(NetBoxModelFilterSet):
    ...
    source_ports = django_filters.CharFilter(
        field_name="source_ports",
        lookup_expr="icontains",
        label=_("Source Ports"),
    )

    # Destination - ТОЛЬКО текстовые поля, НЕТ ForeignKey к Device
    destination_prefix = django_filters.CharFilter(
        field_name="destination_prefix",
        lookup_expr="icontains",
        label=_("Destination Prefix"),
    )
    destination_ports = django_filters.CharFilter(
